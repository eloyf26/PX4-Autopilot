...
<li><b>Self-identification.</b> At boot PG0 powers two resistor ladders; ADC3 reads them on PH4 (module ID) and PH3 (base ID, via X1-27). PX4 then starts exactly the right drivers and axis rotations for this sensor set.</li>
    ...
    parts.append('<p class="note"><b>KiCad: two projects with PCBs.</b> The same data is generated as two KiCad 8 projects in <code>hardware/cuav_v6x_fmum/kicad/</code> on branch <code>claude/unknown-component-schematics-mdgis0</code>: <code>fmum/cuav_v6x_fmum.kicad_pro</code> (FMU module, 4 schematic sheets + a 36 × 31.4 mm 6-layer PCB) and <code>imu/cuav_v6x_imu.kicad_pro</code> (IMU board, schematic + 25 × 18 mm 4-layer PCB). Both boards carry the real outline, holes, connector positions from the Pixhawk standard, datasheet-verified footprints with every pad net-assigned, and photo-derived component placement. They are unrouted: the original inner-layer copper cannot be recovered from photos. The README lists exactly which footprint geometries were approximated.</p>')
